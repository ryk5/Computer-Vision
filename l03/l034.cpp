...
vector<Point> knuth_shuffle(vector<Point> &ve)
{
    unsigned long long size = ve.size();
    srand (time(NULL));
...
unordered_map<pair<unsigned long long, unsigned long long>, Point, hashpair> makeDict(vector<Point>& ve, unsigned long long n, double slength)
{
    unordered_map<pair<unsigned long long, unsigned long long>, Point, hashpair> dict;
    for(unsigned long long i = 0; i < n; i++)
    {
        unsigned long long xcoord = (unsigned long long)(ve[i].getX() / slength);
        unsigned long long ycoord = (unsigned long long)(ve[i].getY() / slength);
        dict[make_pair(xcoord, ycoord)] = ve[i];
    }
    return dict;
}


vector<Point> randomalg(vector<Point> &ve){
    double s = ve[0].dist(ve[1]);
    double slength = s / 2;
    Point first = ve[0];
    Point second = ve[1];
    unordered_map<pair<unsigned long long, unsigned long long>, Point, hashpair> dict;
    dict = makeDict(ve, 2, slength);
    for(int i = 0; i < ve.size(); i++)
    {
        ...
        else 
        {
            for(int dx = -2; dx < 3; dx++)
            {
                ...
                {
                    unsigned long long incx = xcoord + dx;
                    unsigned long long incy = ycoord + dy;
                    pair<unsigned long long, unsigned long long> pairindex = make_pair(incx, incy);
                    if(dict.count(pairindex) > 0)
                    { 
                        if(dict[pairindex].dist(ve[i]) < temp)
                        {
                            temp = dict[pairindex].dist(ve[i]);
                            first = ve[i];
                            second = dict[pairindex];
                        } 
                    }
                }
            }
...
void part4()
{
    ofstream out;
    pts = vreadfile();
    vector<Point> a(pts.begin(), pts.end());
    auto st = chrono::steady_clock::now();
    vector<Point> b = knuth_shuffle(a);
    vector<Point> closest = randomalg(b);
    auto en = chrono::steady_clock::now();
    cout << setprecision(21) << "Randomized Pair: " << closest[0].getX() << ", " << closest[0].getY() << " " << closest[1].getX() << ", " << closest[1].getY() << "\n";
    cout << "Randomized Dist: " << closest[0].dist(closest[1]) << "\n";
    cout << "Randomized (ms): " << chrono::duration_cast<chrono::nanoseconds>(en - st).count()/1e6 << "\n";
    out.open("results.txt", fstream::app);
    out << setprecision(21) <<  "Randomized Pair: " << "("<<closest[0].getX() << ", "  << closest[0].getY() << ") (" << closest[1].getX() << ", " << closest[1].getY() << ")\n";
    out << "Randomized Dist: " << closest[0].dist(closest[1]) << "\n";
    out << "Randomized (ms): " << chrono::duration_cast<chrono::nanoseconds>(en - st).count()/1e6 << "\n";
