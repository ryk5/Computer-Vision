...
list<Point> lreadfile()
{
    ...
    {
        a = content.substr(0, content.find(" ") + 1);
        content = content.substr(content.find(" ") + 1);
        b = content.substr(0, content.find("\n") + 1);
...
{
    ofstream out;
    vector<Point> v = vreadfile();
    auto st = chrono::steady_clock::now();
    sort(v.begin(), v.end(), myfunc);
    vector<Point> closest = dc(v);
    cout << "Recursive Closest Pair: " << closest[0].getX() << " "  << closest[0].getY() << " " << closest[1].getX() << " " << closest[1].getY() << "\n";
    cout << "Recursive Dist: " << closest[1].dist(closest[0]) << "\n";
    auto en = chrono::steady_clock::now();
    cout << "Recursive (s): " << chrono::duration_cast<chrono::nanoseconds>(en - st).count()/1e9 << "\n";
    out.open("results.txt", fstream::app);
    out << "Recursive Closest Pair: " << closest[0].getX() << " "  << closest[0].getY() << " " << closest[1].getX() << " " << closest[1].getY() << "\n";
    out << "Recursive Dist: " << closest[0].dist(closest[1]) << "\n";
    out << "Recursive (s): " << chrono::duration_cast<chrono::nanoseconds>(en - st).count()/1e9 << "\n";
...
void part1()
{
    ...
    cout << "Brute Force Closest Pair: " << first.getX() << " "  << first.getY() << " " << second.getX() << " " << second.getY() << "\n";
    cout << "Brute Force Dist: " << first.dist(second) << "\n";
    auto end = chrono::steady_clock::now();
    cout << "Brute Force (s): " << chrono::duration_cast<chrono::nanoseconds>(end - start).count()/1e9  << "\n";
    ofstream pout;
    pout.open("results.txt");
    pout << "Brute Force Closest Pair: " << first.getX() << " "  << first.getY() << " " << second.getX() << " " << second.getY() << "\n";
    pout << "Brute Force Dist: " << first.dist(second) << "\n";
    pout << "Brute Force (s): " << chrono::duration_cast<chrono::nanoseconds>(end - start).count()/1e9  << "\n";
