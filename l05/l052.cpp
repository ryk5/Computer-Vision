...
{
    readppm(infile);
    readmakegrayscale(false);
    hysteresis(low, high, outfile);
}
void part3(string infile, string outfile, unsigned int low, unsigned int high)
{
    readppm(infile);
}
int main(int argc, char** argv)
{
    if(argc > 1)
    {
        part2(argv[2], argv[8], stoll(argv[4]), stoll(argv[6]));
        //part1(argv[2]);
    }
    else
    {
        part2("image.ppm", "image1.ppm", 80, 200);
    }
}