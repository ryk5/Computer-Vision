...
pair<bool, double> isedges(int** grid, int xcoord, int ycoord, int radius, int pthreshold)
{
    ...
    for(unsigned int x = 0; x <= xmax + 2; x++)
    {
        ...
        if((x + xi) < rows && (xi - x) > 0 && (y + yi) < cols && (yi - y) > 0 && (y + xi) < rows && (xi - y) > 0 && (x + yi) < cols && (yi - x) > 0)
        {
            total += 8;
    ...
    double a = count / (1.0000000000 * total);
    double b = pthreshold / 100.000000000;
    if(a > b)
    {
...
bool ispenny(int a, int b)
{
    ...
    int gtotal = 0;
    int btotal = 0;
    for(int i = a - 40; i <= a + 40; i++)
    {
        for(int j = b - 40; j <= b + 40; j++)
        {
            pixels++;
            rtotal += ppm[a][b][0];
            gtotal += ppm[a][b][1];
...
//     double diff1 = abs(rf - 85) + abs(gf - 85) + abs(bf - 75);
//     double diff2 = abs(rf - 100) + abs(gf - 120) + abs(bf - 50);
    return (abs(rf - ((gf + bf) / 2)) > 10 && (rf + gf + bf) < 700) || (abs(rf - 52) + abs(gf - 50) + abs(bf - 42)) < 40;
}
...
bool iscenter(int a, int b)
{
    ...
    int total = 0;
    int count = 0;
    for(int i = a - 60; i <= a + 60; i++)
    {
        for(int j = b - 60; j <= b + 60; j++)
        {
            if(abs(ppm[i][j][0] - 196) + abs(ppm[i][j][1] - 203) + abs(ppm[i][j][2] - 205) < 10)
            {
                count++;
            }
            total++;
...
void detection(string img, string grayimg, string hyimg, string nonmaximg, string bothimg, string vimg, string ccimg, int threshold1, int threshold2, int cthreshold, int pthreshold, string circleimg, string coinimg, string results, int minrad, int maxrad, int dime, int penny, int nickel, int quarter, int half)
{
    cout << "Checkpoint!" << "\n";
    readppm(img);
    cout << rows << ", " << cols << "\n";
    ...
    cout << "Checkpoint-!" << "\n";
    cout << "potential: " << grayppm[0][0][0] << "\n";
    int** allgx;
    allgx = new int* [rows];
    for(unsigned int i = 0; i < rows; i++)
    {
        allgx[i] = new int[cols];
    }
    int** allgy;
    allgy = new int* [rows];
    for(unsigned int i = 0; i < rows; i++)
    {
        allgy[i] = new int[cols];   
    }   
    cout << "Checkpoint!" << "\n";
    ...
    cout << "Centers: " << centersandrads.size() << "\n";
    int** origins2;
...
//     for (int i = 0; i < (int)(centersandrads.size()); i++) {
//         bool shouldAdd = true;
//         for (int j = 0; j < (int)(centersandrads.size()); j++) {
//             if (i==j) continue;
//             if (i!= j && centersandrads[i].second==centersandrads[j].second && centersandrads[i].first.first == centersandrads[j].first.first && centersandrads[i].first.second == centersandrads[j].first.second) {
//                 shouldAdd=false;
//                 j=(int)(centersandrads.size());
//             }
//             //abs(circles[i].second - circles[j].second) < 55 && 
//             else if (abs(centersandrads[i].first.second - centersandrads[j].first.second) < 78 && abs(centersandrads[i].first.first - centersandrads[j].first.first) < 78 && (centersandrads[j].second.second > centersandrads[i].second.second || (centersandrads[j].second.second == centersandrads[i].second.second && j < i))) {
// //                 cout << "First: r=" << circles[i].second << ", x=" << circles[i].first.first << ", y=" << circles[i].first.second << endl;
// //                 cout << "Second: r=" << circles[j].second << ", x=" << circles[j].first.first << ", y=" << circles[j].first.second << endl;
// //                 cout << endl;
//                 shouldAdd = false;
//                 j = (int)(centersandrads.size());
//             } else {
// //                 doNotAdd.push_back(j);
//             }
//         }
//         if (shouldAdd){ //(level == 3 || centersandrads[i].second < 118)) {
//             filtered.push_back(centersandrads[i]);
//         }
    //}
    ...
    {
        bool mx = true;
        int range = max(i, (int)centersandrads.size() - i);
        for(int j = 1; j < range; j++)
        {
            if(i + j < (int)centersandrads.size())
            {
                if(distsquare(centersandrads[i].first.first, centersandrads[i].first.second, centersandrads[i + j].first.first, centersandrads[i + j].first.second) < centersandrads[i + j].second.first *  centersandrads[i].second.first && centersandrads[i + j].second.second > centersandrads[i].second.second)
                {
                    mx = false;
                    break;
                }
        ...
        if(mx)
        {
            filtered.push_back(make_pair(make_pair(centersandrads[i].first.first, centersandrads[i].first.second), make_pair(centersandrads[i].second.first, centersandrads[i].second.second)));
        }
    }
    ...
    cout << "Filtered: " << filtered.size() << "\n";
    
    
    for(pair<pair<int, int>, pair<int, double>> p : filtered)
    {
    ...
    for(pair<pair<int, int>, pair<int, double>> p : filtered)
    {
        ...
        int colo = 0;
        bool tempbool = false;
        if(zx < dime && !ispenny(ax, bx)) //dime
        {
            colo = 3;
            dimecount+=1;
            tempbool = true;
        }
        else if(zx < penny && ispenny(ax, bx)) //penny
        {
            colo = 1;
            pennycount+=1;
            tempbool = true;
        }
        else if(zx < nickel) //nickel
        {
            colo = 2;
            nickelcount+=1;
            tempbool = true;
        }
        else if(zx < quarter) //quarter
        {
            colo = 4;
            quartercount+=1;
            tempbool = true;
        }
        else if(zx < half) //half dollar
        {
            colo = 5;
            halfcount+=1;
            tempbool = true;
...
{
    //./l063 -f myimg.ppm -lt 150 -ht 200 -ff myimagef.ppm -TC 300 -fv myimagev.ppm -fcc myimageCC.ppm -TCircle 55 -fCi myimageCircles.ppm -fCo myimageCoins.ppm -fR myresults.txt
    ...
    int threshold1 = 100, threshold2 = 150, cthreshold = 22, pthreshold = 20, minrad = 80, maxrad = 120, dime = 87, penny = 95, nickel = 103, quarter = 131, half = 146; //thresholds I found for easy image
    ...
    cout << "Checkpoint!" << "\n";
    detection(img, grayimg, hyimg, nonmaximg, bothimg, vimg, ccimg, threshold1, threshold2, cthreshold, pthreshold, circleimg, coinimg, results, minrad, maxrad, dime, penny, nickel, quarter, half);
}
